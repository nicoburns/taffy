...
pub(super) fn place_grid_items<'a, ChildIter, ChildId: ChildIdBounds>(
    ...
    ChildIter: Iterator<Item = (usize, ChildId, &'a Style)>,
{
    ...
    let map_child_style_to_origin_zero_placement = {
        ...
        move |(index, node, style): (usize, ChildId, &'a Style)| -> (_, _, _, &'a Style) {
...
fn record_grid_placement<ChildId: ChildIdBounds>(
    cell_occupancy_matrix: &mut CellOccupancyMatrix,
    items: &mut Vec<GridItem<ChildId>>,
    node: ChildId,
    index: usize,
...
mod tests {
    ...
    mod test_placement_algorithm {
        ...
        fn placement_test_runner(
            ...
            let mut items: Vec<GridItem<Node>> = Vec::new();
