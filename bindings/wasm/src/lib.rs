...
fn _option_from_f32(value: f32) -> Option<f32> {
    if value.is_nan() {
        None
    } else {
        Some(value)
...
impl Node {
    #[wasm_bindgen(constructor)]
    pub fn new(tree: &TaffyTree, style: &JsValue) -> Self {
        Self { tree: tree.clone(), node: tree.taffy.borrow_mut().new_leaf(parse_style(style)).unwrap() }
    }
