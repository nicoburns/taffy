...
use super::types::{GridTrack, GridTrackKind, TrackCounts, MAX_GRID_TRACKS};
...
pub(crate) fn compute_explicit_grid_size_in_axis(
    ...
    // Saturating arithmetic is used throughout so that pathologically large repetition counts cannot overflow.
    let non_auto_repeating_track_count = template
        ...
        .map(|track_def| match track_def {
            ...
            GenericGridTemplateComponent::Repeat(repeat) => match repeat.count() {
                RepetitionCount::Count(count) => count.saturating_mul(repeat.track_count()),
    ...
    if auto_repetition_count == 0 {
        return (0, non_auto_repeating_track_count.min(MAX_GRID_TRACKS));
    }
    ...
    let num_repetitions: u16 = match auto_fit_container_size {
        ...
        Some(inner_container_size) => {
            ...
            let first_repetition_and_non_repeating_tracks_used_space = non_repeating_track_used_space
                ...
                + (non_auto_repeating_track_count.saturating_add(repetition_track_count).saturating_sub(1) as f32
                    * gap_size);
            ...
            if first_repetition_and_non_repeating_tracks_used_space > inner_container_size {
                ...
                // Note: `num_repetition_that_fit` can be infinite (e.g. zero-sized repeated tracks), in which case
                // the float-to-int cast saturates to `u16::MAX`; saturating_add then keeps it from overflowing.
    ...
    let grid_template_track_count =
        non_auto_repeating_track_count.saturating_add(repetition_track_count.saturating_mul(num_repetitions));
    // Clamp the explicit track count so that it cannot overflow `i16` when converted to grid line coordinates.
    (num_repetitions, grid_template_track_count.min(MAX_GRID_TRACKS))
}
...
pub(super) fn initialize_grid_tracks(
    ...
    // The number of tracks generated by single track definitions and fixed-count repetitions.
    // Saturating arithmetic is used so that pathologically large repetition counts cannot overflow.
    ...
    // The total number of explicit tracks is clamped (see `compute_explicit_grid_size_in_axis` and the
    // clamping in `compute_grid_layout`), but the track template itself is iterated verbatim below. Cap the
    // number of explicit tracks we actually generate at `counts.explicit` so we never create more tracks than
    // the clamped count (which would overflow the track-count bookkeeping below).
    let explicit_track_end_index = counts.negative_implicit as usize + counts.explicit as usize;
        ...
        if let Some(track_template) = track_template {
            track_template.for_each(|track_sizing_function| {
                match track_sizing_function {
                    GenericGridTemplateComponent::Single(sizing_function) => {
                        if current_track_index >= explicit_track_end_index {
                            return;
                        }
                    ...
                    GenericGridTemplateComponent::Repeat(repeat) => match repeat.count() {
                        RepetitionCount::Count(count) => {
                            ...
                            let track_iter =
                                track_iter.cycle().take((repeat.track_count() as usize).saturating_mul(count as usize));
    ...
    let grid_area_tracks = (counts.negative_implicit + counts.explicit).saturating_sub(current_track_index as u16);

    // Create positive implicit tracks
    let positive_implicit_track_count = counts.positive_implicit.saturating_add(grid_area_tracks);
    if auto_track_count == 0 {
        let iter = core::iter::repeat(TrackSizingFunction::AUTO);
        create_implicit_tracks(tracks, positive_implicit_track_count, iter, gap)
    } else {
