...
pub fn compute<NodeRef: LayoutNode>(
    tree: &mut NodeRef,
    known_dimensions: Size<Option<f32>>,
    parent_size: Size<Option<f32>>,
    available_space: Size<AvailableSpace>,
    ...
    let get_child_styles_iter = || tree.children().map(|child_node: NodeRef::ChildId| tree.child_style(child_node));
    ...
    let mut items: Vec<GridItem<NodeRef>> = Vec::with_capacity(tree.child_count());
